...
namespace Aventi\Servientrega\Model\Sync;

use Aventi\Servientrega\Helper\Email;
...
class Guide
{
    ...
    /**
     * Bridge functions to separate Servientrega response and sync Magento statuses.
     * @param $arrayOrders
     * @param $xmlResponse
     */
    public function bridgeStatus($arrayOrders, $xmlResponse)
    {
        $response = simplexml_load_string($xmlResponse->EstadoGuiasXMLResult->any);
        $json = json_encode($response);
        $xmlArray = json_decode($json, true);
        try {
            foreach ($xmlArray['EstadosGuias'] as $value) {
                foreach ($arrayOrders as $arrayOrder) {
                    if (in_array($value['Guia'], $arrayOrder)) {
                        $this->updateStatus($value, $arrayOrder);
                    }
                }
            }
        } catch (Exception $e) {
    ...
    public function updateStatus($guideInfo, $orderInfo)
    {
        try {
            ...
            if ($status == Order::STATE_COMPLETE || $status == self::SHIPPING) {
                ...
                } elseif ($guideInfo['IdEstadoEnvio'] == 3) {
                    $order->addCommentToStatusHistory(
                        __("Your order has been delivered. Delivery date: {$guideInfo['Fecha_Entrega']}"),
                        self::DELIVERED,
                        true
                    );
                    $this->orderRepository->save($order);
