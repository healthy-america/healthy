 ...
 * @class Product
 */
class ProductFast implements Cron
{
    /**
     * @constructor
     *
    ...
    {
        $this->logger->info("Cronjob Sincronice Product fast is executed.");
        try {
            $this->manageProduct->process(['fast' => true]);
        } catch (Exception $e) {
            $this->logger->debug('Error in product cronjob: ' . $e->getMessage());
        }
        $this->logger->info("Cronjob Sincronice Product fast is finished.");
